...
def ReadCommentsSH5V(f, fontsize):
    ...
    for i, comment in enumerate(comment_element["root"]["bgs"]):
        try:
            ...
            if c_type != '7':
                yield (
                    float(c_at), int(c_date), i, c, {'0': 0, '1': 0, '4': 2, '5': 1}[c_type], int(c_color[1:], 16),
                    size,
                    (c.count('\n') + 1) * size, CalculateLength(c) * size)
            else:
                c_x = float(comment['x'])
...
CommentFormatMap = {None: None, 'Niconico': ReadCommentsNiconico, 'Acfun': ReadCommentsAcfun,
                    'Bilibili': ReadCommentsBilibili, 'Tudou': ReadCommentsTudou, 'Tudou2': ReadCommentsTudou2,
                    'MioMio': ReadCommentsMioMio, 'sH5V': ReadCommentsSH5V}
...
def ConvertFlashRotation(rotY, rotZ, X, Y, width, height):
    ...
    trX = (X * math.cos(rotZ) + Y * math.sin(rotZ)) / math.cos(rotY) + (1 - math.cos(rotZ) / math.cos(
            rotY)) * width / 2 - math.sin(rotZ) / math.cos(rotY) * height / 2
...
def ProcessComments(comments, f, width, height, bottomReserved, fontface, fontsize, alpha, duration_marquee,
                    duration_still, reduced, progress_callback):
...
def Danmaku2ASS(input_files, output_file, stage_width, stage_height, reserve_blank=0,
                ...
                duration_still=5.0, is_reduce_comments=False, progress_callback=None):
    fo = None
    ...
    try:
        ...
        ProcessComments(comments, fo, stage_width, stage_height, reserve_blank, font_face, font_size, text_opacity,
                        duration_marquee, duration_still, is_reduce_comments, progress_callback)
    finally:
...
def main():
    ...
    Danmaku2ASS(args.file, args.output, width, height, args.protect, args.font, args.fontsize, args.alpha,
                args.duration_marquee, args.duration_still, args.reduce)
